...
df = pd.read_csv("risposte_questionario_5_short.csv", index_col=False)
...
fig, ax = plt.subplots(figsize=(6, len(selected_questions) * 0.7))
    ...
    for value in ordered_values:
        if value in counts_df.index:
            ...
            if width > 0:
                ...
                ax.text(bottom[j] + width / 2, f'Q{int(j)+1}', value, va='center', ha='center', color='white', fontsize=9, fontweight='bold')
...
# Imposta i tick dell'asse x ai valori da 1 a 19
ax.set_xticks(np.arange(1, 20))

# Imposta le etichette dei tick dell'asse x
...
ax.grid(True, which='major', axis='x', linestyle='--', color='grey', alpha=0.7, zorder=0)  # Griglia principale
#ax.grid(True, which='minor', axis='x', linestyle=':', color='lightgrey', alpha=0.7, zorder=0)  # Sottogriglia
...
ax.set_xlabel('Number of Responses', fontsize=10)  # Etichetta asse x
ax.set_ylabel('Question ID', fontsize=12)  # Etichetta asse y
