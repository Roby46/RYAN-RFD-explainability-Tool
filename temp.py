...
context_spec='''In relational databases, a Relaxed Functional Dependency (RFD) is an integrity constraint X -> Y 
   ...
   lower than the similarity threshold defined on that attribute. The function to assess similarity is edit distance for strings and absolute difference for numbers.
...
context_gen='''In relational databases, a Relaxed Functional Dependency (RFD) is an integrity constraint X -> Y 
   ...
   X is named Left Hand Side (LHS), while Y is the Right Hand Side (RHS). Two values of an attribute are similar if their distance is 
   lower than the similarity threshold defined on that attribute. The function to assess similarity is edit distance for strings and absolute difference for numbers.
   After the deletion of a tuple, an existing RFD can be no longer minimal. Indeed, a generalized RFD with a subset of attributes on the LHS may be valid on the datase 
   if the deleted tuple had similar values on the LHS with respect other tuples but it had different values on the RHS. 
'''


detail_gen='''
   You will be provided with an RFD that gets generalized after the deletion of a batch of tuples, and with the tuples that caused the generalization and their values. 

   Generalized RFD = LHS: ["Journal(threshold=1)", "Author(threshold=3)"], RHS: ["Volume(threshold=2)"]. 

   This happened because:
   - Tuple 1 (Journal=Nature, Author=Cahill, Volume=4), which caused a violation considering tuple 3 (Journal=Natures, Author=Wei, Volume=16), has been removed.
   - Tuple 2 (Journal=Science,  Author=Sung, Volume=10), which caused a violation considering tuple 4 (Journal=Sciences, Author=Lynn, Volume=1) has been removed.
